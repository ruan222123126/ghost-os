import { deleteSkill, listSkills, updateSkill } from './api';
...
describe('lib/api/skills/api', () => {
  ...
  it('updateSkill calls PATCH /api/skills/:id', async () => {
    const expected: SkillPayload = {
      id: 'skill_repo_release',
      name: 'release',
      description: 'repo release skill',
    ...
    const skill = await updateSkill('skill_repo_release', { enabled: false });
