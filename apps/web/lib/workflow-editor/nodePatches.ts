...
type WorkflowGroupDraft = NonNullable<WorkflowCanvasNodeDraft['group']>;
...
export function withGroupNode(
  node: WorkflowCanvasNodeDraft,
  patch: Partial<WorkflowGroupDraft>,
): WorkflowCanvasNodeDraft {
  const group = node.group;
  return {
    ...node,
    group: {
      title: groupFieldValue(patch.title, group?.title, ''),
      shared_context: groupFieldValue(patch.shared_context, group?.shared_context, ''),
      speaking_mode: groupFieldValue(
        patch.speaking_mode,
        group?.speaking_mode,
        DEFAULT_GROUP_SPEAKING_MODE,
      ),
      owner_agent_id: groupFieldValue(patch.owner_agent_id, group?.owner_agent_id, ''),
      max_rounds: groupFieldValue(patch.max_rounds, group?.max_rounds, DEFAULT_GROUP_MAX_ROUNDS),
    },
  };
}

function groupFieldValue<T>(patchValue: T | undefined, currentValue: T | undefined, defaultValue: T): T {
  return patchValue ?? currentValue ?? defaultValue;
}
