  ...
  it('renders enabled tools before disabled tools', () => {
    const html = renderToolList({
      tools: [
        ...
        { name: 'screen_control', enabled: true },
    ...
    const screenControlIndex = html.indexOf('screen_control');
    ...
    expect(screenControlIndex).toBeGreaterThan(-1);
    expect(webSearchIndex).toBeGreaterThan(-1);
    expect(codexCLIIndex).toBeGreaterThan(-1);
    expect(scriptExecIndex).toBeLessThan(screenControlIndex);
    expect(screenControlIndex).toBeLessThan(webSearchIndex);
