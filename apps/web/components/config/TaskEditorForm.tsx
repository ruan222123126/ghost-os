...
export function TaskEditorForm(props: TaskEditorFormProps) {
  ...
  const submitLabel = resolveSubmitLabel(editorMode, saving);
    ...
    <form className="space-y-8" onSubmit={(event) => handleTaskSubmit(event, onSubmit)}>
      <div>
        <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.15em] text-[#737373]">
          {editorMode === 'edit' ? 'Editing Task' : 'New Text Task'}
        </p>
        <h3 className="text-[20px] font-medium text-[#111111]">
          {editorMode === 'edit' ? 'Edit Text Task' : 'Create Text Task'}
        </h3>
      </div>
...
function resolveSubmitLabel(mode: TaskEditorMode, saving: boolean): string {
  if (saving) {
    return 'Saving...';
  }
  if (mode === 'edit') {
