...
func (d *DecisionService) selectRecipeReuse(input recipeSelectionInput) (*DecisionRecipe, *RecipeSelectionReport, *RecipeAdvisory) {
	...
	for _, hit := range recipeHits {
		...
		candidate := d.scoreRecipeSelectionCandidate(recipe, hit, input, now)
...
func (s *QueryService) decorateRecipeSelection(result MemoryQueryResult, query MemoryQuery, scope SessionScope) MemoryQueryResult {
	...
	selectedRecipe, report, advisory := s.decision.selectRecipeReuse(recipeSelectionInput{
		Query:        query,
		Scope:        scope,
		Hits:         result.DecisionHits,
		Entries:      result.Entries,
		Plan:         result.IntentPlan,
		RerankReport: result.RerankReport,
		Now:          time.Now().UTC(),
	})
