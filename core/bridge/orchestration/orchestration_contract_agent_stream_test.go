...
func TestValidateAgentResponsePayloadRejectsResponseMode(t *testing.T) {
	err := validateAgentResponsePayload(agentResponse{
		Message:      "final answer",
		SessionID:    "session-1",
		SessionEnded: false,
	...
	if err.Error() != "agent response mode is not supported" {
...
func TestPrepareAgentTurnRequestRejectsPlanMode(t *testing.T) {
	_, err := prepareAgentTurnRequest(agentParams{
		Mode:    " PLAN ",
	...
	if err.Error() != `unsupported agent mode: "plan"` {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestExecuteAgentActionRejectsPlanMode(t *testing.T) {
	...
	if code := bus.StatusFromError(err); code != 400 {
		t.Fatalf("unexpected status code: got %d want %d", code, 400)
	}
	if !strings.Contains(err.Error(), `unsupported agent mode: "plan"`) {
		t.Fatalf("unexpected error: %v", err)
	}
}
