...
func TestValidateTaskDefinitionWorkflowRejectsInvalidCases(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong edge direction", task: ScheduledTask{TaskKind: taskKindWorkflow, Workflow: &WorkflowDefinition{Nodes: []WorkflowNode{{ID: "start-node", Type: "start"}, {ID: "end-node", Type: "end"}}, Edges: []WorkflowEdge{{FromNodeID: "end-node", ToNodeID: "start-node"}}}}, want: "start node must have in=0 and out=1"},
