...
func sortedProModeToolNames(extra map[string]tools.Tool) []string {
...
func buildProModeUserPrompt(request proModeRequest, records []session.IterationRecord, iteration int) string {
	limitLine := buildProModeLimitLine(request)
	history := buildProModeHistory(records)
...
func buildProModeLimitLine(request proModeRequest) string {
	if request.Mode == proModePro {
...
func buildProModeHistory(records []session.IterationRecord) string {
	if len(records) == 0 {
		return "(none yet)"
	}
