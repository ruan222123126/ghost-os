...
func buildRelayModeSystemPrompt(basePrompt string, relay taskdefs.TaskRelayConfig) string {
	modeRule := "End every round by calling `relay_update_record`; do not end with plain text."
	if relay.StopPolicy == taskdefs.RelayStopPolicyAIDecides {
	...
	if relay.StopPolicy == taskdefs.RelayStopPolicyMaxRounds {
		modeRule = fmt.Sprintf(
...
func buildRelayModeUserPrompt(task string, records []session.RelayRecord, round int, relay taskdefs.TaskRelayConfig) string {
...
func buildRelayModeRepairPrompt(previousOutput string, relay taskdefs.TaskRelayConfig) string {
...
func buildRelayModeProtocolErrorPrompt(errorText string, previousOutput string, relay taskdefs.TaskRelayConfig) string {
	requiredTool := "`relay_update_record`"
	if relay.StopPolicy == taskdefs.RelayStopPolicyAIDecides {
		requiredTool = "`relay_update_record` or `relay_complete`"
	}
...
func relayMaxRoundsLine(relay taskdefs.TaskRelayConfig) string {
