...
func (d RoundDispatcher) dispatchSequential(
	...
	for _, agentID := range cmd.ParticipantIDs {
		...
		if result.Status == taskdefs.RunStatusSuccess {
...
func (d RoundDispatcher) runParallelMember(
	...
	if cmd.results[cmd.round].Status != taskdefs.RunStatusSuccess {
		cancel()
	}
}
...
func (d RoundDispatcher) runMember(
	...
	if d.Members == nil {
		return member.SetupError(req, errors.New("orchestration member runner is not configured"))
	}
