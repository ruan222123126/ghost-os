...
func (b *graphBuilder) connect(edge Edge) error {
	...
	if fromID == "" || toID == "" {
		return fmt.Errorf("%w: workflow edge endpoints are required", taskdefs.ErrInvalidTaskConfig)
	}
	if fromID == toID {
		return fmt.Errorf("%w: workflow does not allow self-loop edge %q", taskdefs.ErrInvalidTaskConfig, fromID)
	}
	if _, ok := b.index.nodes[fromID]; !ok {
		return fmt.Errorf("%w: workflow edge references unknown from_node_id %q", taskdefs.ErrInvalidTaskConfig, fromID)
	}
	if _, ok := b.index.nodes[toID]; !ok {
		return fmt.Errorf("%w: workflow edge references unknown to_node_id %q", taskdefs.ErrInvalidTaskConfig, toID)
	}
...
func (b *graphBuilder) connectKnownNodes(fromID string, toID string) error {
	key := fromID + "->" + toID
	if b.edgeSet[key] {
		return fmt.Errorf("%w: duplicate workflow edge %q", taskdefs.ErrInvalidTaskConfig, key)
	}
	b.edgeSet[key] = true
