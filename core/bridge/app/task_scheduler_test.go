...
func TestTaskSchedulerStartSkipsCorruptedTasks(t *testing.T) {
	...
	scheduler.execute = func(_ context.Context, task ScheduledTask, _ string) scheduledTaskExecutionResult {
		select {
		case ran <- task:
		default:
		}
		return scheduledTaskExecutionResult{Status: taskRunStatusSuccess, SessionIDOutput: "session-ok"}
	}
