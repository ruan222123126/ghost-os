...
func TestRunReturnsIterationHandoff(t *testing.T) {
	...
	completer := newFakeCompleter(newToolCallsResponse(newToolCall("call-relay", "relay_update_record", `{"did":"inspected config","remaining":"apply patch"}`)))
	agent := newTestAgent(completer, newFakeToolCatalog(tool), 2)

	_, err := agent.Run(context.Background(), "continue relay work")
	if err == nil {
