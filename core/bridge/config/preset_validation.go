...
type promptLibraryByID map[string]SystemPromptLibraryItem
...
func buildPromptLibraryByID(library []SystemPromptLibraryItem) promptLibraryByID {
...
func normalizePresetToolAllowlist(raw []string, index int) ([]string, error) {
	normalized := normalizeConfiguredToolNames(raw)
...
func normalizePresetPromptRefs(
	...
	if err := validatePresetPromptRef(index, "rule", rule, SystemPromptInsertPointRule, library); err != nil {
		return PresetPromptRefs{}, err
	}
	if err := validatePresetPromptRef(index, "core_job", coreJob, SystemPromptInsertPointCoreJob, library); err != nil {
		return PresetPromptRefs{}, err
	}
	if err := validatePresetPromptRef(index, "memory", memory, SystemPromptInsertPointMemory, library); err != nil {
		return PresetPromptRefs{}, err
	}
...
func normalizePresetContextPromptRefs(
	...
	for refIndex, rawRef := range refs {
		...
		if err := validatePresetPromptRef(index, slot, refID, SystemPromptInsertPointContext, library); err != nil {
			return nil, err
		}
		seen[refID] = struct{}{}
