...
func migrateAndNormalizeSystemPromptFiles(files SystemPromptFiles) (SystemPromptFiles, bool, error) {
	...
	if len(normalized.PromptLibrary) == 0 && trimSystemPromptValue(&normalized.CorePrompt) != "" {
		normalized.PromptLibrary = buildCoreJobPromptLibrary(&normalized.CorePrompt)
		changed = true
	}
	if len(normalized.PromptLibrary) == 0 {
		trimmedCorePrompt := trimSystemPromptValue(&normalized.CorePrompt)
		if normalized.CorePrompt != trimmedCorePrompt {
			normalized.CorePrompt = trimmedCorePrompt
			changed = true
		}
		return normalized, changed, nil
	}

	library, err := normalizePromptLibrary(normalized.PromptLibrary)
	if err != nil {
		return SystemPromptFiles{}, false, err
	}
	if !promptLibraryEqual(normalized.PromptLibrary, library) {
		changed = true
	}
	normalized.PromptLibrary = library

	compiledCorePrompt := compileCorePromptFromLibrary(normalized.PromptLibrary)
	if normalized.CorePrompt != compiledCorePrompt {
		normalized.CorePrompt = compiledCorePrompt
		changed = true
	}
	return normalized, changed, nil
}
...
func promptLibraryEqual(left []SystemPromptLibraryItem, right []SystemPromptLibraryItem) bool {
