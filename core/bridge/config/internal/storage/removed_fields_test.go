...
func TestLoadRejectsLegacyProviderFields(t *testing.T) {
...
func TestLoadRejectsRemovedGraphQLFields(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.toml")
	t.Setenv("GHOST_CONFIG_PATH", configPath)
...
func TestLoadRejectsRemovedToolPromptOverrides(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.toml")
	t.Setenv("GHOST_CONFIG_PATH", configPath)

	if err := os.WriteFile(configPath, []byte(`tool_prompt_overrides = { script_exec = "old" }`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	_, _, err := Load()
	if err == nil {
		t.Fatal("expected removed tool prompt overrides config error")
	}
	if !strings.Contains(err.Error(), "tool_prompt_overrides") {
