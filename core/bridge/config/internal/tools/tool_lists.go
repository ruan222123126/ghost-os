...
func ToolNameListOrDefault(raw []string, defaultCSV string) []string {
	if raw != nil {
		return normalizeConfiguredToolNames(raw)
	}
	return normalizeConfiguredToolNames(parseStringCSV(defaultCSV))
}
...
func ValidConfiguredToolNames() map[string]bool {
	return validConfiguredToolNames()
}
