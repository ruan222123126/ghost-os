...
func TestToolSelector_FallbackOnErrorCases(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "low confidence",
			completer: &fakeSelectorCompleter{response: selectorResponse(`{"mode":"subset","tools":["script_exec"],"confidence":0.40,"reason":"too weak"}`)},
			cfg:       newSelectorTestConfig(),
		},
		{
			name:      "unknown tool",
...
func TestToolSelector_BuildPromptIncludesDecisionHintSection(t *testing.T) {
	selector := NewToolSelector(newSelectorTestConfig(), &fakeSelectorCompleter{})
	prompt := selector.buildSelectorPrompt("patch config.go", []llm.Message{{Role: llm.RoleUser, Text: "inspect config first"}}, "Similar successful cases used: script_exec")
	if !strings.Contains(prompt, "Prior similar experience:\nSimilar successful cases used: script_exec") {
...
func TestToolSelector_SelectToolsPassesDecisionHintToWorker(t *testing.T) {
	...
	hint := "Similar successful cases used: script_exec"
